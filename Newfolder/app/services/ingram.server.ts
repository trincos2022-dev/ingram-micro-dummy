...
const freightEstimateCache = createTtlCache<string, {
...
export async function saveCredentials(shopDomain: string, data: CredentialInput) {
  const sandboxFlag =
...
export async function requestFreightEstimate(
  ...
  const requestPromise = (async () => {
    const credentials = opts?.credentials ?? (await getCredentialsOrThrow(shopDomain));
    ...
    const billToAddressId = payload.billToAddressId || credentials.billToAddressId || "000";
    const shipToAddressId = payload.shipToAddressId || credentials.shipToAddressId || "200";
 ...
 */
export async function getCarrierConfigurations(shopDomain: string): Promise<CarrierConfig[]> {
 ...
 */
export async function getEnabledCarrierCodes(shopDomain: string): Promise<Set<string>> {
  const configs = await prisma.carrierConfiguration.findMany({
 ...
 */
export async function hasCarrierConfigurations(shopDomain: string): Promise<boolean> {
