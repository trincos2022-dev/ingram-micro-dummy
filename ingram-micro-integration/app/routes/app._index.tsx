...
async function findExistingCarrierService(
  shop: string,
  accessToken: string,
) {
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    const latestProductSyncJobRecord = await getLatestProductSyncJob(session.shop);
    ...
    switch (intent) {
      ...
      case "registerCarrierService": {
        ...
        return {
          ok: true,
          intent,
          message: "Carrier service registered. Shopify will now call the app for rates.",
        } satisfies ActionResult;
      }
      ...
      case "saveCarrierConfig": {
        ...
        for (const config of carrierConfigs) {
          const isEnabled = formData.get(`carrier_${config.carrierCode}`) === "on";
          updates.push({
      ...
      case "saveFallbackRate": {
        ...
        await saveFallbackRateSettings(session.shop, {
          ...
          description: description ? String(description) : "Please contact support before placing this order",
        });
...
export default function Index() {
  ...
  return (
    <s-page heading="Ingram Micro freight rates">
      {statusMessage && (
        <div
          style={{
            padding: "1rem",
      ...
      <s-stack direction="block" gap="base">
        ...
        <s-section heading="Connection health">
          <s-paragraph>
            Make sure Shopify can exchange the stored client credentials for an
            access token before wiring the checkout flow.
          </s-paragraph>
        ...
        <s-section heading="SKU → Ingram mapping sync">
          <s-paragraph>
            SKU → Ingram part number mappings are synced from Supabase for fast
            checkout lookups. <strong>Auto-syncs every Monday at 3:00 AM UTC.</strong>
          </s-paragraph>
          <s-stack direction="block" gap="base">
            <div
              style={{
            ...
            >
              <syncFetcher.Form method="post" action="/api/sync-products">
                ...
                >
                  {syncIsRunning ? "Sync in progress..." : "Sync now (manual)"}
                </s-button>
              </syncFetcher.Form>
            ...
            >
              ...
              {(syncStatus === "running" || syncTotal > 0) && (
                <div style={{ marginTop: "0.5rem" }}>
                  ...
                  >
                    <div
                      style={{
                        width: `${syncPercent}%`,
                        height: "100%",
                        background: syncStatus === "success" ? "#059669" : "#3b82f6",
                  ...
                  >
                    <span>
                      {syncProcessed.toLocaleString()} / {syncTotal.toLocaleString() || "?"} SKUs mapped
                    </span>
              ...
              {currentSyncJob?.error ? (
                <div style={{ marginTop: "0.5rem", padding: "0.5rem", background: "#fef2f2", borderRadius: "4px", color: "#dc2626", fontSize: "0.875rem" }}>
        ...
        <s-section heading="Checkout carrier service">
          ...
          {carrierService ? (
            <s-stack direction="block" gap="base">
              ...
              <Form method="post">
                <input type="hidden" name="_action" value="deleteCarrierService" />
                <input
                  type="hidden"
          ...
          {carrierConfigs.length > 0 ? (
                  ...
                  {carrierConfigs.map((config) => (
                    ...
                    >
                      ...
                      <span>
                        ...
                        <br />
                        <small style={{ color: "var(--color-text-subdued, #6d7175)" }}>
                          {config.carrierCode} • {config.carrierMode || "Standard"}
                        </small>
                      </span>
                    </label>
                  ))}
            ...
            >
              <s-text>
                ...
                mappings. You can also use the freight estimate sandbox below to
                populate available carriers.
              </s-text>
            </s-box>
          )}
        </s-section>
        ...
        <s-section heading="Fallback shipping rate">
          <s-paragraph>
            When Ingram cannot provide shipping rates (missing SKU mapping, API errors, etc.),
            this fallback rate will be shown at checkout instead of Shopify's default backup rate.
            Set a high price to prevent accidental orders.
          </s-paragraph>
          ...
          <Form method="post">
            ...
            <s-stack direction="block" gap="base">
              ...
              >
                ...
                />
                <label htmlFor="fallbackEnabled" style={{ cursor: "pointer" }}>
                  <strong>Enable fallback rate</strong>
                  <br />
                  <small style={{ color: "#6b7280" }}>
                    When disabled, empty rates are returned and Shopify's backup rate will be used
                  </small>
                </label>
              </div>
              ...
              <s-text-field
                ...
                onInput={(event: any) =>
                  setFallbackForm((prev) => ({
                    ...prev,
                    title: event.target?.value || event.currentTarget?.value || "",
                  }))
                }
              />
              ...
              >
                <small style={{ color: "#6b7280" }}>Preview at checkout:</small>
            ...
            <s-stack direction="block" gap="base">
              ...
              <div>
                ...
                <textarea
                  ...
                  name="linesJson"
                  value={linesInput}
                  onChange={(event) => setLinesInput(event.currentTarget.value)}
                  rows={10}
                  style={{ width: "100%" }}
                  required
                />
