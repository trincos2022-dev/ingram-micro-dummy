...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    const allRows: { price_vendor_part: string; ingram_part_number: string }[] = [];
    let offset = 0;
    ...
    while (true) {
      ...
      const validRows = (data ?? []).filter(
        (row): row is { price_vendor_part: string; ingram_part_number: string } =>
          Boolean(row.price_vendor_part) && Boolean(row.ingram_part_number)
      );
    ...
    for (const row of rows) {
      if (!uniqueMappings.has(row.price_vendor_part)) {
        uniqueMappings.set(row.price_vendor_part, row.ingram_part_number);
      }
    }

    console.log(`[Cron] ${uniqueMappings.size} unique SKUs after deduplication`);
